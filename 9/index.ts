...
export type ApiResponse<T> =  (
    ...
    /*T extends IUser ? ( { status: 'success'; data: IUser[]; } | { status: 'error'; error: string; } )
    : T extends IAdmin ? ( { status: 'success'; data: IAdmin[]; } | { status: 'error'; error: string; } )
    : never;*/

export function requestAdmins(callback: (response: ApiResponse<IAdmin[]>) => void): void {
    callback({
        status: 'success',
        data: admins
    });
...
export function requestUsers(callback: (response: ApiResponse<IUser[]>) => void): void {
    callback({
        status: 'success',
        data: users
    });
...
export function requestCoffeeMachineQueueLength(callback: (response:  ApiResponse<number>) => void): void {
    callback({
        status: 'error',
...
function startTheApp(callback: (error: Error | null) => void): void {
    requestAdmins((adminsResponse: ApiResponse<IAdmin[]>): void => {
        console.log('Admins:');
        ...
        requestUsers((usersResponse: ApiResponse<IUser[]>): void => {
            console.log('Users:');
            ...
            requestCurrentServerTime((serverTimeResponse: any): void => {
                ...
                requestCoffeeMachineQueueLength((coffeeMachineQueueLengthResponse: any): void => {
