...
const isAdmin: (person: Person) => person is IAdmin = (person: Person): person is IAdmin => person.type === 'admin';
const isUser: (person: Person) => person is IUser = (person: Person): person is IUser => person.type === 'user';

function logPerson(person: Person, index: number): void {
    ...
    let registeredAt: string = dateWizard(person.registered, '{date}.{month}.{year} {hours}:{minutes}');
    let num: string = `#${dateWizard.pad(index + 1)}`;
    console.log(
...
([] as Person[])
    .concat(users, admins)
    .filter((person: Person) => dateWizard.dateDetails(person.registered).hours < 10)
    .forEach(logPerson);
