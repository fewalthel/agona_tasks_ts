...
function logUser(user: IUser | null): void {
    if (!user) {
        console.log(' - none');
        return;
    }
...
function logAdmin(admin: IAdmin | null): void {
    if (!admin) {
        console.log(' - none');
        return;
    }
...
const compareUsers:(a: IUser, b: IUser ) => number = (a: IUser, b: IUser): number => a.age - b.age;
const compareAdmins:(f: IAdmin, b: IAdmin) => number = (a: IAdmin, b: IAdmin): number => a.age - b.age;
const colorizeIndex: (value: number) => string = (value: number): string => String(value + 1);
...
console.log(` - ${String(getAverageValue(users, ({age}: IUser): number => age))} years`);
...
console.log(` - ${String(getAverageValue(admins, ({age}: IAdmin): number => age))} years`);
